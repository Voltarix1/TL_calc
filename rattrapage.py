...
def parse_exp0(l):
    try:
        ...
        if get_current() in [V_T.SEQ, V_T.END]:
            return "problème"
        return parse_exp5(l)
...
def parse_exp1p(l, n1):
    if get_current() == V_T.POW:
        ...
        n2 = parse_exp1(l) 
        return pow(n1, n2) if n2 != "problème" else n1
    return n1
...
def parse_Y2(l, n1):
    ...
    if get_current() == V_T.MUL:
        ...
        n2 = parse_exp3(l)
        n = parse_Y2(l, n1*n2) if n2 != "problème" else n1
...
def parse_Y3(l, n1):
    ...
    if get_current() in [V_T.ADD, V_T.SUB]:
        if get_current() == V_T.ADD:
            ...
            n2 = parse_exp4(l)
            n = parse_Y3(l, n1+n2) if n2 != "problème" else n1
        else:
            consume_token(V_T.SUB)
            n2 = parse_exp4(l)
            n = parse_Y3(l, n1-n2) if n2 != "problème" else n1
...
def recover(suiv):
    while get_current() not in suiv and get_current() != V_T.END and get_current() != V_T.SEQ:
        print(f"{get_current()} ignoré.")
